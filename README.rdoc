...
Sample usage from an IRB console, using a GeoServer installation with a 'druid' workspace.

  % rake console
  > require 'rgeoserver'
  > catalog = RGeoServer.catalog
  => RGeoServer::Catalog: http://localhost:8080/geoserver/rest
  > w = catalog.get_workspace 'druid'
  => RGeoServer::Workspace: druid (new?: false)
...
- <b>v1.0</b> - Major rewrite built on Rails 4.0
